...
const EllipseGradient: React.FC<{ color: string }> = ({ color }) => (
  <div className="absolute inset-0 z-[5] flex h-full w-full items-center justify-center">
    <svg width="100%" height="100%" viewBox="0 0 356 180" preserveAspectRatio="none" fill="none" xmlns="http://www.w3.org/2000/svg">
      <rect width="356" height="180" fill="url(#v2_paint)" />
      <defs>
        <radialGradient id="v2_paint" cx="0" cy="0" r="1" gradientUnits="userSpaceOnUse" gradientTransform="translate(178 98) rotate(90) scale(98 178)">
...
const Layer1: React.FC<LayerProps> = ({ hovered, color, secondaryColor }) => {
  ...
  // Reset to the idle progress the moment hover ends. Done in render
  // (not an effect) to avoid set-state-in-effect; the hover-in animation
  // still runs from an effect because it is time-delayed.
    ...
    <div className="ease-[cubic-bezier(0.6, 0.6, 0, 1)] absolute inset-0 z-[6] flex w-full translate-y-0 items-start justify-center bg-transparent p-4 transition-transform duration-500 group-hover/animated-card:translate-y-full">
      <div className="ease-[cubic-bezier(0.6, 0.6, 0, 1)] rounded-md border border-zinc-200 bg-white/25 px-2 py-1.5 opacity-100 backdrop-blur-sm transition-opacity duration-500 group-hover/animated-card:opacity-0 dark:border-zinc-800 dark:bg-black/25">
        <div className="flex items-center gap-2">
          ...
          <p className="text-xs text-black dark:text-white">Call Analytics Overview</p>
        </div>
        <p className="mt-1 text-xs text-neutral-500 dark:text-neutral-400">Voice latency & resolution stats.</p>
      </div>
    </div>
  </div>
)
...
const Layer3: React.FC<{ color: string }> = ({ color }) => (
  <div className="ease-[cubic-bezier(0.6, 0.6, 0, 1)] absolute inset-0 z-[6] flex translate-y-full items-center justify-center opacity-0 transition-all duration-500 group-hover/animated-card:translate-y-0 group-hover/animated-card:opacity-100">
    <svg width="100%" height="100%" viewBox="0 0 356 180" preserveAspectRatio="none" fill="none" xmlns="http://www.w3.org/2000/svg">
      <rect width="356" height="180" fill="url(#v2_linear)" />
      <defs>
        <linearGradient id="v2_linear" x1="178" y1="0" x2="178" y2="180" gradientUnits="userSpaceOnUse">
...
const Layer4: React.FC<LayerProps> = ({ color, secondaryColor, hovered }) => {
  const items = [
    { id: 1, translateX: "100", translateY: "50", text: "STT" },
    { id: 2, translateX: "100", translateY: "-50", text: "TTS" },
    { id: 3, translateX: "125", translateY: "0", text: "NLU" },
    { id: 4, translateX: "-125", translateY: "0", text: "LLM" },
    { id: 5, translateX: "-100", translateY: "50", text: "<500ms" },
    { id: 6, translateX: "-100", translateY: "-50", text: "WebRTC" },
  ]
    ...
    <div className="ease-[cubic-bezier(0.6, 0.6, 0, 1)] absolute inset-0 z-[7] flex items-center justify-center opacity-0 transition-opacity duration-500 group-hover/animated-card:opacity-100">
      {items.map((item, index) => (
        ...
        >
          ...
          <span className="ml-1 text-[10px] text-black dark:text-white">{item.text}</span>
        </div>
      ))}
    </div>
  )
