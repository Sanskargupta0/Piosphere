  ...
  return (
    <div className="absolute inset-0 z-[5] flex h-full w-full items-center justify-center">
      <svg width="100%" height="100%" viewBox="0 0 356 180" preserveAspectRatio="none" fill="none" xmlns="http://www.w3.org/2000/svg">
        <rect width="356" height="180" fill="url(#v1_paint)" />
        <defs>
          <radialGradient id="v1_paint" cx="0" cy="0" r="1" gradientUnits="userSpaceOnUse" gradientTransform="translate(178 98) rotate(90) scale(98 178)">
...
// Double-wide bar chart that slides left on hover - must stay 200% so translate-x-[-50%] works
const Layer1 = ({ color, secondaryColor }: LayerProps) => {
  return (
    <div className="ease-[cubic-bezier(0.6,0.6,0,1)] absolute top-0 left-0 z-[6] w-[200%] transform transition-transform duration-500 group-hover/animated-card:translate-x-[-50%]">
      <svg className="w-full" viewBox="0 0 712 180" preserveAspectRatio="xMidYMid meet" fill="none" xmlns="http://www.w3.org/2000/svg">
...
// Line chart covering left half - slides away on hover via the gradient overlay
const Layer2 = ({ color }: LayerProps) => {
  return (
    <div className="absolute top-0 left-[-1px] h-full w-1/2">
      <svg className="h-full w-full" viewBox="0 0 356 180" preserveAspectRatio="none" fill="none" xmlns="http://www.w3.org/2000/svg">
        <g clipPath="url(#v1_clip)">
...
const Layer3 = ({ color, secondaryColor }: LayerProps) => {
  return (
    <div
      ...
      style={{ "--color": color, "--secondary-color": secondaryColor } as React.CSSProperties & { "--color": string; "--secondary-color": string }}
    >
      <div className="flex shrink-0 items-center rounded-full border border-zinc-200 bg-white/25 px-1.5 py-0.5 backdrop-blur-sm transition-opacity duration-300 ease-in-out group-hover/animated-card:opacity-0 dark:border-zinc-800 dark:bg-black/25">
        ...
        <span className="ml-1 text-[10px] text-black dark:text-white">Agents</span>
      </div>
      <div className="flex shrink-0 items-center rounded-full border border-zinc-200 bg-white/25 px-1.5 py-0.5 backdrop-blur-sm transition-opacity duration-300 ease-in-out group-hover/animated-card:opacity-0 dark:border-zinc-800 dark:bg-black/25">
        <div className="h-1.5 w-1.5 rounded-full bg-[var(--secondary-color)]" />
        <span className="ml-1 text-[10px] text-black dark:text-white">Workflows</span>
      </div>
    </div>
  )
}
    ...
    <div className="group relative h-full w-full">
      <div className="ease-[cubic-bezier(0.6,0.6,0,1)] absolute inset-0 z-[7] flex w-full -translate-y-full items-start justify-start bg-transparent p-4 transition-transform duration-500 group-hover/animated-card:translate-y-0">
        <div className="ease-[cubic-bezier(0.6,0.6,0,1)] rounded-md border border-zinc-200 bg-white/25 p-1.5 opacity-0 backdrop-blur-sm transition-opacity duration-500 group-hover/animated-card:opacity-100 dark:border-zinc-800 dark:bg-black/25">
          <p className="mb-1 text-xs font-semibold text-black dark:text-white">Agent Performance Metrics</p>
          <p className="text-xs text-neutral-500 dark:text-neutral-400">Real-time task execution analytics.</p>
        </div>
      </div>
    </div>
  )
